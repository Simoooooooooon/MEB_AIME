...
class MainWindow(QtWidgets.QMainWindow):
    def __init__(self):
        ...
        self.dev=nidaqmx.system.Device("dev2")       
    ...
    def file_save(self):
        ...
        chemin = 'C:/Users/tp_meb/Desktop'
        file_name= QtWidgets.QFileDialog.getSaveFileName(parent=self,caption='Save image', directory= chemin, filter='Image File (*.tif)')
    ...
    def affiche_image(self):
        ...
        if self.magnification >= 200000 :
            self.error = QtWidgets.QMessageBox.warning(self, 'Text Error Dialog', 'Entrer un grandissement inferieur à 200 000')   
    ...
    def eventFilter(self, object, event):
        ...
        if object == self.scene and event.type() == QtCore.QEvent.GraphicsSceneMousePress:
            spf = event.scenePos()
        ...
        if object == self.scene and event.type() == QtCore.QEvent.GraphicsSceneMouseRelease:
            spf = event.scenePos()
            lpf = self.pixmap_item.mapFromScene(spf)
            brf = self.pixmap_item.boundingRect()
            if brf.contains(lpf):
        ...
        if object == self.scene and event.type() == QtCore.QEvent.GraphicsSceneMouseMove:
